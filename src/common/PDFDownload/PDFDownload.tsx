...
import logo from "../../assets/logo.png"; // Adjust as per your structure
...
const PDFDownload: React.FC<Props> = ({
  ...
  const { name, employee_id, designation, unit_name } = data?.data || {};
  ...
  const savePDF = async () => {
    ...
    let metaY = 45;
    doc.text(`Generated By: ${name || "N/A"}`, 10, metaY);
    metaY += 5;
    doc.text(`Employee ID: ${employee_id || "N/A"}`, 10, metaY);
    metaY += 5;
    doc.text(`Designation: ${designation || "N/A"}`, 10, metaY);
    metaY += 5;
    doc.text(`Unit Name: ${unit_name || "N/A"}`, 10, metaY);
    metaY += 5;
    doc.text(`Date: ${date_time}`, 10, metaY);
